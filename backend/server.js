...
app.get('/getAll', (req,res)=>{
    conexion.query('SELECT * FROM coches', (error,filas)=>{
        if(error){
            throw error
        }else{
            res.send(filas)
...
//????????????mirar
app.get('/:id', (req,res)=>{
    conexion.query('SELECT * FROM coches WHERE id = ?', [req.params.id], (error, fila)=>{
        if(error){
            throw error
        }else{
            res.send(fila)
...
app.get('/search/:name', (req, res) => {
    ...
    const query = 'SELECT * FROM coches WHERE marca = ?';
...
app.post('/insert', (req,res)=>{
    ...
    let data = {marca:req.body.marca, modelo:req.body.modelo, ano:req.body.ano, precio:req.body.precio, color:req.body.color}
    console.log(data)
...
app.put('/update', (req, res)=>{
    ...
    let id = req.body.id
    let nombre = req.body.marca
    let modelo = req.body.modelo
    let ano= req.body.ano
    let precio= req.body.precio
    let color= req.body.color
    
    let sql = "UPDATE coches SET marca = ?, modelo = ?, ano = ?, precio = ?, color = ? WHERE id = ?"
...
app.delete('/delete/:id', (req,res)=>{
    conexion.query('DELETE FROM coches WHERE id = ?', [req.params.id], function(error, filas){
        if(error){
            throw error
        }else{              
            res.send(filas)
